...
public class Login extends AppCompatActivity implements GoogleApiClient.OnConnectionFailedListener {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == SIGN_IN_CODE ){
            GoogleSignInResult reult = Auth.GoogleSignInApi.getSignInResultFromIntent(data);
            handleSignInResult(reult);
        }
    }

    private void handleSignInResult(GoogleSignInResult reult) {
        if (reult.isSuccess()){
            firebaseAuthWithGoogle(reult.getSignInAccount());
        }else{
        ...
        firebaseAuth.signInWithCredential(credential).addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
            ...
            public void onComplete(@NonNull Task<AuthResult> task) {
                ...
                if (!task.isSuccessful()){
                    Toast.makeText(getApplicationContext(), "No se pudo auntenticar con Firebase", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
